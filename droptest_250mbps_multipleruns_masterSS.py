...
with open('MasterDropPercentageAndBandwidth.csv', 'w', newline = '') as f:
    # write the header
    writer = csv.writer(f)
...
for numbers in range(2):
    ...
    for cxs in cx:
        ...
        f.close()
        ff = open('MasterDropPercentageAndBandwidth.csv', 'a', newline = '')
        data2 = [current_drop_percentage, current_bandwidth_usage]
        writer_two = csv.writer(ff)
        writer_two.writerow(data2)
        ff.close()
        index += 1
